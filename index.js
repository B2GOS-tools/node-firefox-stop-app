...
module.exports = function(options) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    if (manifestURL === undefined || client === undefined) {
      return reject(new Error('App manifestURL and client are required to close an app'));
    }

    getWebAppsActor(client).then(function(webAppsActor) {
      closeApp(webAppsActor, manifestURL)
...
function closeApp(webAppsActor, appManifestURL) {
  return new Promise(function(resolve, reject) {
    webAppsActor.close(appManifestURL, function(err, response) {
      if (err) {
        return reject(err);
      }
      resolve(response);
